...
async function getFollowers(userID) {
  const options = {
    ...
    params: {
      id_user: userID,
      limit: 100 // Request 100 followers per page
    },
    headers: {
      'X-RapidAPI-Key': 'c07cad21f6mshf988cf17f6a6e9ep1d9e64jsn8e9bdafc25cc',
      'X-RapidAPI-Host': 'instagram-scraper-2022.p.rapidapi.com'
  ...
  try {
    while (true) {
      ...
      followers.forEach(follower => {
        if (!uniqueUsernames.has(follower.username)) {
          allFollowers.push(follower);
          uniqueUsernames.add(follower.username);
        }
      });

      if (followers.length < 100) {
        break; // Exit the loop if the response contains fewer than 100 followers
      }

      page++; // Increment the page number to fetch the next page of followers
    }

    const usernames = allFollowers.map(follower => follower.username);
    usernames.sort();
